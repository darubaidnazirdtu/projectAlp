...
import { set, get, getByActivityId, getByDepartmentId, update, deleteOutreach } from "../data-access/extension_outreach_activities.data-access.js"

const enterData = asyncHandler(async (req, res) => {
  const data = req.body

    // Auto-injected JSON parsing for object lists
    ;["faculty_associations","student_associations","external_contributors"].forEach(field => {
  ...
  const requiredFields = ['department_id', 'title_of_activity', 'type_of_activity', 'start_date', 'end_date', 'location', 'academic_year', 'year_of_sanction', 'target_beneficiaries'];
...
const getById = asyncHandler(async (req, res) => {
  ...
  const response = await getByActivityId(id)
  if (!response) {
