...
import { set, get, getByProgramId, getByDepartmentId, getByFacultyId, update, deleteProgram } from "../data-access/faculty_development_programs.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Auto-injected JSON parsing for object lists
    ;["faculty_participants","external_participants"].forEach(field => {
    ...
    const requiredFields = ['department_id', 'program_title', 'type_of_program', 'mode', 'start_date', 'end_date', 'academic_year', 'year_of_sanction', 'outcome'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByProgramId(id)
    if (!response) {
