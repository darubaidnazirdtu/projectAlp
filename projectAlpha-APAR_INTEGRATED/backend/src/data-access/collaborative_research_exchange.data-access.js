...
/**
 * Collaborative Research Exchange Data Access Layer
 * Updated to use Collaboration collection with type='exchange'
 */
...
const set = async (data, loggedInUser = null) => {
  try {
    ...
    // Trigger auto-sync
    const transformed = transformExchange(saved.toObject());
    ...
    if (facultyIds.length > 0) {
      ...
      const entryData = { type: 'exchange', ...transformed };
...
const get = async () => {
  try {
    const collabs = await Collaboration.find({ type: 'exchange' }).sort({ start_date: -1 });
    return collabs.map(c => transformExchange(c.toObject()));
  } catch (error) {
    throw error;
  }
}

const getByCollaborationId = async (collaboration_id) => {
  try {
    const collab = await Collaboration.findOne({ collaboration_id, type: 'exchange' });
    return collab ? transformExchange(collab.toObject()) : null;
  } catch (error) {
    throw error;
  }
}
...
const getByDepartmentId = async (department_id) => {
  try {
    const collabs = await Collaboration.find({ department_id, type: 'exchange' }).sort({ start_date: -1 });
    return collabs.map(c => transformExchange(c.toObject()));
  } catch (error) {
    throw error;
  }
...
const update = async (collaboration_id, data, loggedInUser = null) => {
  try {
    const updateFields = {};
    if (data.department_id) updateFields.department_id = data.department_id;
    if (data.type_of_activity) {
      updateFields.type_of_activity = data.type_of_activity;
      updateFields.title = data.type_of_activity;
    }
    if (data.name_of_institution) updateFields.name_of_institution = data.name_of_institution;
    ...
    if (data.nature_of_collaboration) updateFields.nature_of_collaboration = data.nature_of_collaboration;
    ...
    const result = await Collaboration.findOneAndUpdate(
      { collaboration_id, type: 'exchange' },
      {
        $set: updateFields,
        $push: {
          'metadata.change_log': {
    ...
    // Trigger auto-sync
    if (result) {
      const transformed = transformExchange(result.toObject());
      const facultyIds = transformed.faculty_associations.map(f => f.faculty_id).filter(Boolean);
      if (facultyIds.length > 0) {
        let ay = transformed.academic_year;
        if (!ay && transformed.year) ay = `${transformed.year}-${transformed.year + 1}`;
...
const deleteExchange = async (id) => {
  try {
    const query = { type: 'exchange' };
    if (mongoose.Types.ObjectId.isValid(id)) {
      query.$or = [{ collaboration_id: id }, { activity_id: id }, { _id: id }];
    } else {
      query.$or = [{ collaboration_id: id }, { activity_id: id }];
    }

    const collab = await Collaboration.findOne(query);
    if (!collab) {
      console.warn(`[DELETE EXCHANGE] Record not found for ID: ${id}`);
      throw new Error(`Research Exchange record not found for ID: ${id}`);
    }

    const transformed = transformExchange(collab.toObject());
    const facultyIds = transformed.faculty_associations.map(f => f.faculty_id).filter(Boolean);
    ...
    const entryData = { type: 'exchange', action: 'deleted', ...transformed };
...
export { set, get, getByCollaborationId, getByDepartmentId, update, deleteExchange }