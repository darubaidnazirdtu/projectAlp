...
import { set, get, getByPatentId, getByDepartmentId, getByStatus, getByCountry, addFacultyToPatent, addStudentToPatent, removeFacultyFromPatent, removeStudentFromPatent, deletePatent, update } from "../data-access/patents.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Auto-injected JSON parsing for object lists
    ;["faculty_members","students","external_inventors"].forEach(field => {
    ...
    const requiredFields = ['patent_title', 'application_number', 'status', 'date_of_filing', 'department_id'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByPatentId(id)
    if (!response) {
