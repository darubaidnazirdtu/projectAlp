...
import { set, get, getByAwardId, getByDepartmentId, getByFacultyId, update, deleteAward } from "../data-access/research_innovation_awards.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Auto-injected JSON parsing for object lists
    ;["faculty_recipients","student_recipients","external_recipients"].forEach(field => {
    ...
    // FIX: Just parse the JSON. 
    if (typeof data.faculty_recipients === 'string') {
        try {
            data.faculty_recipients = JSON.parse(data.faculty_recipients);
        } catch (e) {
            data.faculty_recipients = [];
        }
    }

    // FIX: Same for students
    if (typeof data.student_recipients === 'string') {
        try {
            data.student_recipients = JSON.parse(data.student_recipients);
        } catch (e) {
            data.student_recipients = [];
        }
    }

    // Validation based on tableConfig required fields
    const requiredFields = ['department_id', 'name_of_award', 'type_of_award', 'category_of_award', 'level', 'name_of_organisation', 'awarding_agency', 'year', 'date_of_award', 'academic_year'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByAwardId(id)
    if (!response) {
