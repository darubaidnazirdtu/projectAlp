...
import { set, get, getByConsultancyId, getByDepartmentId, update, deleteConsultancy } from "../data-access/revenue_from_consultancy.data-access.js"
...
const enterData = asyncHandler(async (req, res) => {
    ...
    if (typeof data.faculty_involved === 'string') {
        try {
            data.faculty_involved = JSON.parse(data.faculty_involved);
        } catch (e) {
            data.faculty_involved = [];
        }
    }
    if (typeof data.students_involved === 'string') {
        try {
            data.students_involved = JSON.parse(data.students_involved);
        } catch (e) {
    ...
    const requiredFields = ['department_id', 'name_of_project', 'agency_name', 'type_of_agency', 'grant_amount', 'revenue_generated', 'duration_start_date', 'end_date', 'year_of_consultancy', 'status'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByConsultancyId(id)
    if (!response) {
