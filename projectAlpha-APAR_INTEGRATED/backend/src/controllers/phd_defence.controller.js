...
import { set, get, getByDefenceId, getByDepartmentId, getBySupervisorId, getByStudentId, getByAcademicYear, update, deleteDefence } from "../data-access/phd_defence.data-access.js"
...
const enterData = asyncHandler(async (req, res) => {
    ...
    if (typeof data.co_supervisors === 'string') {
        try {
            data.co_supervisors = JSON.parse(data.co_supervisors);
        } catch (error) {
            data.co_supervisors = [];
        }
    }

    if (typeof data.committee_members === 'string') {
        try {
            data.committee_members = JSON.parse(data.committee_members);
        } catch (error) {
    ...
    const requiredFields = ['student_id', 'enrollment_no', 'student_name', 'department_id', 'thesis_title', 'thesis_type', 'supervisor_id', 'supervisor_name', 'date_of_defence', 'result_outcome', 'academic_year'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByDefenceId(id)
    if (!response) {
