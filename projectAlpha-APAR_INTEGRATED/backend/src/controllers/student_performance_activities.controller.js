...
import { set, get, getByPerformanceId, getByDepartmentId, update, deletePerformance } from "../data-access/student_performance_activities.data-access.js";

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Auto-injected JSON parsing for object lists
    ;["students","external_participants"].forEach(field => {
    ...
    const requiredFields = ['department_id', 'name_of_award', 'type_of_activity', 'level_of_award', 'category', 'position_or_rank', 'organizing_body', 'year', 'academic_year'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByPerformanceId(id)
    if (!response) {
