...
import { set, get, getByMentorRecordId, getByDepartmentId, getByFacultyId, update, deleteMentoring } from "../data-access/mentors_stress_support.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Validation based on tableConfig required fields
    const requiredFields = ['faculty_id', 'department_id', 'activity_type', 'academic_year', 'semester', 'number_of_participants', 'target_group', 'mode', 'date_of_activity', 'organizer_name', 'mentor_name', 'outcome'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByMentorRecordId(id)
    if (!response) {
