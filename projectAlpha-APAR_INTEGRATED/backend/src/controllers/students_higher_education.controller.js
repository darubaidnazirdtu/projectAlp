...
import { set, get, getByRecordId, getByDepartmentId, getByStudentId, update, deleteRecord } from "../data-access/students_higher_education.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Validation based on tableConfig required fields
    const requiredFields = ['student_id', 'department_id', 'student_name', 'institution_joined', 'country', 'level_of_study', 'programme_type', 'details_of_programme', 'academic_year', 'year_of_joining', 'current_status'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByRecordId(id)
    if (!response) {
