...
import { set, get, getByExamRecordId, getByDepartmentId, getByStudentId, update, deleteExam } from "../data-access/students_competitive_exams.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Validation based on tableConfig required fields
    const requiredFields = ['student_id', 'department_id', 'name_of_exam', 'type_of_exam', 'level_of_exam', 'result_status', 'academic_year'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByExamRecordId(id)
    if (!response) {
