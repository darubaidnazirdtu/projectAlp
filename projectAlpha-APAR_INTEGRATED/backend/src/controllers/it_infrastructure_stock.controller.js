...
import { set, get, getByStockId, getByDepartmentId, update, deleteStock } from "../data-access/it_infrastructure_stock.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Validation based on tableConfig required fields
    const requiredFields = ['department_id', 'lab_name', 'faculty_id', 'year_of_installation', 'condition_status', 'usage_purpose'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByStockId(id)
    if (!response) {
