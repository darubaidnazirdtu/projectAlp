...
import { set, get, getByMethodId, getByDepartmentId, getByFacultyId, getByCourseId, update, deleteMethod } from "../data-access/student_centric_methods.data-access.js"

const enterData = asyncHandler(async (req, res) => {
    const data = req.body

    // Validation based on tableConfig required fields
    const requiredFields = ['course_id', 'programme_id', 'course_name', 'programme_name', 'department_id', 'faculty_id', 'academic_year', 'semester', 'level', 'method_type', 'details_of_methods'];
...
const getById = asyncHandler(async (req, res) => {
    ...
    const response = await getByMethodId(id)
    if (!response) {
